...
#  Array ArmorItems[3].tag.ucit.boomber.nbt.input[0]: a copy of array you want to replace an element,
#  Any ArmorItems[3].tag.ucit.boomber.nbt.input[1][0]: the element you want to replace into an array,
#  Score #bb.nbt.input.index: replace index
# )
#
...
execute as @s[scores={bb.success=0}] if score #bb.nbt.index bb.calculation matches 0.. run function boomber:advanced_nbt/array/set/positive
execute as @s[scores={bb.success=0}] if score #bb.nbt.index bb.calculation matches ..-1 run function boomber:advanced_nbt/array/set/negative
